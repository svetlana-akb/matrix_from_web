...
def generate_result_list(g_matrix, left_top_x, left_top_y, right_down_x, right_down_y, result_list):
    ...
    while j > left_top_y:
        ...
        j -= 1
    return generate_result_list(g_matrix, left_top_x + 1, left_top_y + 1, right_down_x - 1, right_down_y - 1,
                                result_list)
...
matrix_url = sys.argv[1] if len(sys.argv) > 1 else input("Please input url:\n")
asyncio.run(get_matrix(matrix_url))
input("\nPlease press some key for end of program\n")
